...
func BenchmarkTableDispatch(b *testing.B) {
	metric70, _ := encoding.NewPlain(false, false).Load([]byte("abcde_fghij.klmnopqrst.uv_wxyz.1234567890abcdefg 12345.6789 1234567890"), make(encoding.Tags))
...
func BenchmarkTableDisPatchAndEndpointReceive(b *testing.B) {
	...
	metric70, _ := encoding.NewPlain(false, false).Load([]byte("abcde_fghij.klmnopqrst.uv_wxyz.1234567890abcdefg 12345.6789 1234567890"), make(encoding.Tags)) // size: key = 48, val = 10, ts = 10 -> 70
